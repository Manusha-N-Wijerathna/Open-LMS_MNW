...
export default function AdminDashboard() {
    ...
    const handleRejectUser = async (id: string, name: string) => {
        setConfirmDialog({
            ...
            message: `Are you sure you want to reject or revoke verification for ${name || 'this user'}?`,
                                                        ...
                                                        <tr key={u.id} className="hover:bg-white/[0.01] transition-colors group">
                                                            ...
                                                            <td className="px-6 py-4 text-right">
                                                                {u.is_verified ? (
                                                                    <button
                                                                        onClick={() => handleRejectUser(u.id, u.full_name || '')}
                                                                        className="text-xs font-bold text-slate-500 hover:text-red-400 px-3 py-1.5 rounded-lg hover:bg-red-500/5 border border-transparent hover:border-red-500/10 transition-all opacity-0 group-hover:opacity-100"
                                                                    >
                                                                        Revoke Access
                                                                    </button>
                                                                ) : (
                                                                    <button
                                                                        onClick={() => handleVerifyUser(u.id, u.full_name || '')}
                                                                        className="text-xs font-bold bg-green-500/10 hover:bg-green-500 text-green-400 hover:text-white border border-green-500/20 hover:border-transparent px-3 py-1.5 rounded-lg transition-all"
                                                                    >
                                                                        Approve Access
                                                                    </button>
                                                                )}
